...
async function run() {
    try {
        ...
        if (!fs.existsSync(configPath)) {
            throw new Error(`config.json not found in content path: ${resolvedContentPath}`);
        }
...
async function deployToGitHubPages(outputPath, githubToken) {
    try {
        ...
        try {
            ...
            const { stdout: statusOutput } = await exec.getExecOutput("git", ["status", "--porcelain"], { cwd: repoDir });
        ...
        core.error(`❌ Failed to deploy to GitHub Pages: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
}
